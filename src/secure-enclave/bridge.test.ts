...
describe.skipIf(process.platform !== 'darwin')('Secure Enclave Bridge Integration', () => {
  beforeAll(async () => {
    const binaryAvailable = await isSecureEnclaveBinaryAvailable()
    if (!binaryAvailable) {
      throw new Error('Secure Enclave binary not found. Run `npm run build:swift` first.')
    }
  })
